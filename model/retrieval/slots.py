...
class ScouterAttention(nn.Module):
    ...
    def forward(self, inputs_pe, inputs, weight=None, things=None, check=None):
        ...
        for _ in range(self.iters):
            ...
            attn2 = attn / (attn.sum(dim=-1, keepdim=True) + 1)
            updates = torch.einsum('bjd,bij->bid', inputs, attn2)

        if self.vis:
            slots_vis_raw = attn.clone()
            out = vis(slots_vis_raw, "vis", self.args.feature_size, weight, things, check)
            return updates, attn, out

        return updates, attn


def vis(slots_vis_raw, loc, size, weight=None, things=None, check=None):
        ...
        if weight is not None:
            ...
            for id, image in enumerate(slots_vis):
                ...
                if check is not None:
                    image.save(check + f'slot_{id:d}.png')
