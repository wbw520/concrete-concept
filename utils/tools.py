...
def predict_hash_code(args, model, data_loader, device):
    ...
    for batch_idx, (data, label) in enumerate(data_loader):
        ...
        if not args.pre_train:
            ...
            #     for i in range(b):
            #         slots_vis = att[i]
            #         slots_vis = (slots_vis - slots_vis.min()) / (slots_vis.max() - slots_vis.min())
            #         if slots_vis.max() < 0.5:
            #             status = 0
            #         else:
            #             status = 1
            #         record.append(slots_vis.unsqueeze(0).sum(-1) * status)
            #     att_new = torch.cat(record, dim=0)
...
def make_grad(p, target_index, extractor, output, img_heat, grad_min_level, save_name):
    x, y = img_heat.size
    ...
    print(mask.shape)
    mask = F.interpolate(mask, size=(y, x), mode="bilinear")
...
def show_cam_on_image(img, masks, target_index, save_name):
    ...
    mask_image.save(f'visualization/{save_name}_{target_index}_mask.png')

    heatmap_only, heatmap_on_image = apply_colormap_on_image(img, final, 'jet')
    heatmap_on_image.save(f'visualization/{save_name}_{target_index}.png')
